...
int main(int argc, char *argv[]) {
	...
	if (my_rank != 0) {
		...
		case 1:
			sprintf(smSong,
					"Adelante San Marcos Glorioso, Adelante tu siempre ...");
			MPI_Send(smSong, strlen(smSong) + 1, MPI_CHAR, 0, 0,
					MPI_COMM_WORLD);
			break;
		case 2:
			myGrade = 20;
		...
		MPI_Recv(&myGrade, sizeof(myGrade), MPI_INT, 2, 0, MPI_COMM_WORLD,
				&estado);
		...
		MPI_Recv(&dollarValue, sizeof(dollarValue), MPI_FLOAT, 4, 0,
				MPI_COMM_WORLD, &estado);
		std::cout << "Proc_4 su mensaje es: " << dollarValue << "\n";
		MPI_Recv(&longValue, sizeof(longValue), MPI_LONG_LONG, 5, 0,
				MPI_COMM_WORLD, &estado);
